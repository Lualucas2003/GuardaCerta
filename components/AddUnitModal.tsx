...
const AddUnitModal: React.FC<AddUnitModalProps> = ({ onClose, onSave }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            await api.createUnit(name, sigla);
            onSave();
            onClose();
        } catch (error) {
            console.error("Failed to create unit", error);
            alert(`Falha ao criar a unidade: ${error instanceof Error ? error.message : String(error)}`);
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="bg-slate-800 border border-slate-700 shadow-2xl w-full max-w-lg rounded-lg transform transition-all">
                <form onSubmit={handleSubmit}>
                    <div className="p-6 border-b border-slate-700">
                        <h2 className="text-xl font-bold text-slate-100">Adicionar Nova Unidade</h2>
                    </div>
